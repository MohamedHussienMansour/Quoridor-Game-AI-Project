...
class GameGUI:
    def __init__(
        ...
        self.cell_side = round((self.game_surface_side - ((game_size + 1) * self.margin)) / self.game_size)
    ...
    def get_hovered_board_position(self, mouse_pos):
        ...
        if not (0 <= x_offset < self.game_surface_side and 0 <= y_offset < self.game_surface_side):
            return None
        ...
        for y in range(len(self.board)):
            for x in range(len(self.board[y])):
                ...
                rect = pygame.Rect(left + self.game_x_pos, top + self.game_y_pos, width, height)
    ...
    def handle_click(self):
        ...
        if self.first_wall_part is None:
            ...
            if (y, x) == (fy, fx):
                ...
                my = (fy + y) // 2
                mx = (fx + x) // 2
                self.board[my][mx] = HORIZONTAL_CONNECTOR_CODE if y == fy else VERTICAL_CONNECTOR_CODE
                self.first_wall_part = None
    ...
    def is_valid_second_part(self, x, y, fx, fy):
        ...
        if fy % 2 == 1 and fx % 2 == 0:
            ...
            if (
                self.board[my - 1][mx] == 1 and
                self.board[my][mx] == 2 and
            ...
            if (
                self.board[my][mx - 1] == 1 and
                self.board[my][mx] == 1 and
    ...
    def map_board_pos(self, x, y):
        left = (x // 2) * (self.cell_side + self.margin) + (x % 2) * self.cell_side + self.margin
        top = (y // 2) * (self.cell_side + self.margin) + (y % 2) * self.cell_side + self.margin
    ...
    def get_wall(self, x, y):
        ...
        height = self.cell_side - self.wall_to_wall_gap if is_vertical_wall else self.margin
        width = self.margin if is_vertical_wall else self.cell_side - self.wall_to_wall_gap
    ...
    def get_wall_connector(self, x, y, is_horizontal_connector, is_vertical_connector):
        ...
        width = self.margin if is_vertical_connector else self.margin + self.wall_to_wall_gap
        height = self.margin if is_horizontal_connector else self.margin + self.wall_to_wall_gap
    ...
    def draw_board(self):
        ...
        for y in range(len(self.board)):
            for x in range(len(self.board[y])):
                ...
                is_horizontal_wall, is_vertical_wall = self.check_wall_type(x, y)
                is_connector = not (is_cell or is_vertical_wall or is_horizontal_wall) and self.board[y][x] > 0
                ...
                if is_cell:
                    ...
                    pygame.draw.rect(surface, pygame.Color("#611111"), cell, border_radius=round(self.cell_side*0.1))

                if self.board[y][x] > 0 and is_cell:
                    player_pos = (left + round(self.cell_side/2), top + round(self.cell_side/2))
                    player_color = pygame.Color("#FFAAAA") if self.board[y][x] == PLAYER_1_CODE else pygame.Color("#FF0000")

                    pygame.draw.circle(surface, player_color, player_pos, self.player_radius)
                elif (is_vertical_wall or is_horizontal_wall) and self.board[y][x] > 0:
                    left, top, width, height = self.get_wall(x, y)
                    pygame.draw.rect(surface, pygame.Color("#F3F3F3"), pygame.Rect(left, top, width, height))
                elif is_connector:
                    is_vertical_connector = self.board[y][x] == VERTICAL_CONNECTOR_CODE
                    is_horizontal_connector = self.board[y][x] == HORIZONTAL_CONNECTOR_CODE

                    left, top, width, height = self.get_wall_connector(x, y, is_horizontal_connector, is_vertical_connector)
