    ...
    def rule(self, direction, pos):
        ...
        if direction == "up":
            if pos[0] - 2 < 0:  # player out of Board
                return False
            if (
                Player.board.board[pos[0] - 1, pos[1]] != 0
            ):  # there is wall restricted move
                return False
            if (
                Player.board.board[pos[0] - 2, pos[1]] != 0
            ):  # there is player front of me
                if (
                    pos[0] - 4 < 0
                ):  # player out of Board with respect there is player front of me
                    return False
                if (
                    Player.board.board[pos[0] - 3, pos[1]] != 0
                ):  # there is no wall restricted  to jump double
                    return False
                Player.board.board[self.pos[0], self.pos[1]] = 0
                pos[0] -= 2
                return True
            ...
            if pos[1] + 2 > 16:  # player out of Board
                return False
            if (
                Player.board.board[pos[0], pos[1] + 1] != 0
            ):  # there is wall restricted move
                return False
            if (
            ...
            if (
                Player.board.board[pos[0] - 2, pos[1]] != 0
                or Player.board.board[pos[0], pos[1] - 2] != 0
            ):
                if (
                    Player.board.board[pos[0] - 2, pos[1]] != 0
                ):  # there is player front of me
                    ...
                    if pos[1] - 2 < 0 or pos[0] - 2 < 0:  # player out of Board
                        return False
                    if (
                        Player.board.board[pos[0] - 2, pos[1] - 1] != 0
                    ):  # there is wall restricted move
                        return False
                ...
                if (
                    ...
                    if pos[1] - 2 < 0 or pos[0] - 2 < 0:  # player out of Board
                        return False
                    if (
                        Player.board.board[pos[0] - 1, pos[1] - 2] != 0
                    ):  # there is wall restricted move
                        return False
                return True
            else:
            ...
            if (
                Player.board.board[pos[0] + 2, pos[1]] != 0
                or Player.board.board[pos[0], pos[1] + 2] != 0
            ) and not (pos[1] + 2 > 16 or pos[0] + 2 > 16):
                if (
                    Player.board.board[pos[0] + 2, pos[1]] != 0
                ):  # there is player front of me
                    if (
        ...
        if X1 % 2 != 0:  # first one is odd
            if abs(X1 - X2) == 2 and Y1 == Y2:  # check diff  == 2
                ...
                if (
                    ...
                    and Player.board.board[X2, Y2] == 0
                    and Player.board.board[meanX, Y2] == 0
                ):
            ...
            if abs(Y1 - Y2) == 2 and X1 == X2:  # check diff  == 2
                ...
                if (
                    Player.board.board[X1, Y1] == 0
                    and Player.board.board[X2, Y2] == 0
                    and Player.board.board[X1, meanY] == 0
                ):
                    Player.board.board[meanX, meanY] = -1
                else:
                    print("invalid ")
    ...
    def WallRestrictionAlgorithms(self):
        ...
        for i in range(1, Player.pid + 1):
            ...
            while start or posY + 1 != self.objective:
                ...
                while (
                    Player.board.board[posY, posX] == 0
                    and posY != endY
                    and (posX, posY) not in TraveledState
                ):
                    posY += direction * 2
                if posY == endY:
                    return True
                ...
                for Xchange, end in [-1, -1], [1, 17]:
                    ...
                    while (
                        posX + Xchange != end
                        and Player.board.board[posY - direction, posX + Xchange] == 0
                    ):
                        posX += Xchange * 2
...
if __name__ == "__main__":
    ...
    for i in range(moves):
        ...
        check = int(
            input("Do you want to chech whether the wall placement valid or not ")
        )
        if check == 1:
